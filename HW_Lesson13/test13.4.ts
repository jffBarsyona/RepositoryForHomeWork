...
export function filter<T>(filterer: FilterFunction<T>, input: T[]): T[];
export function filter<T>(filterer: FilterFunction<T>): (input: T[]) => T[];
export function filter<T>(filterer?: FilterFunction<T>, input?: T[]): T[] | ((input: T[]) => T[]) | typeof filter;
...
export function add(a: number, b: number): number | ((subB: number) => number) | typeof add {
    ...
    if (arguments.length === 1) {
        return function subFunction(subB: number): number | ((subB: number) => number) | typeof add {
            if (arguments.length === 0) {
                return subFunction;
            }
            return a + subB;
